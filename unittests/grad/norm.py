...
def general_test(act: bool, psum: bool, zero_mean: bool, samples: int, power: int):  # skipcq: PYL-W0640
    ctx = Context()
    ...
    for trial in range(trials):
        ...
        out0 = grad(lambda x: norm_forward(ctx, x[0], x[1], bool(psum), act)[0])
...
def test_act(act: bool, samples: int):
    general_test(act, False, False, samples, 2)
...
def test_zero_mean(zero_mean: bool, samples: int):
    general_test(True, False, zero_mean, samples, 2)


@pytest.mark.parametrize("power", [1, 2, 3, 4])
@pytest.mark.parametrize("samples", sample_sizes)
def test_power(power: int, samples: int):
    general_test(True, False, False, samples, power)
