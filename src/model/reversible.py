...
def _reversible_at_init(ctx: Context, fn: ReversibleFn, sparse_access: SparseAccess, src: REVERSIBLE_CTX,
                        *args) -> REVERSIBLE_CTX:
    _x00, x01, x10, x11, sparse, d_sparse = src
    ...
    return x10, x11, out, x01, sparse, d_sparse
...
def reversible(ctx: Context, fn: ReversibleFn, sparse_access: SparseAccess, src: REVERSIBLE_CTX,
    ...
    @jax.custom_gradient
    def _fn(params, inputs: REVERSIBLE_CTX, inner_args: List[jax.Array]):
        x0, _dx0, x1, _dx1, sparse, d_sparse = inputs

        def _grad(dy):
            dy0, y0, dy1, y1, dy_sparse, y_sparse = dy
        ...
        if sparse_access == SparseAccess.write:
            ...
            (out,), keys = out
        out = x0 + out
        return (x1, x1, out, out, sparse, sparse), _grad
