...
def prenorm(fn: typing.Callable[[Context, jnp.ndarray], jnp.ndarray]):
    def _fn(ctx: Context, inp: jnp.ndarray) -> jnp.ndarray:
        ...
        out = fn(ctx, inp)
        return out * get_param(ctx, "rezero", [1], mean=0, std=0, dtype=out.dtype)
...
def reversible(ctx: Context, fn: typing.Callable[[Context, jnp.ndarray], jnp.ndarray],
    ...
    def _fn(params: typing.Dict[str, jnp.ndarray], x0: jnp.ndarray, back_x0: jnp.ndarray, x1: jnp.ndarray,
        ...
        def _grad(dy: REVERSIBLE_CTX) -> REVERSIBLE_CTX:
            ...
            update(ctx, {name: grad for name, grad in d_params.items() if '0' not in str(grad.dtype)},
                   back_step.astype(jnp.int32))
