...
def main():
    ...
    def objective(trial: optuna.trial.Trial) -> typing.Optional[float]:
        ...
        ctx.optimizer.warmup_end = trial.suggest_int("warmup_end", 16, 8192, log=True)
        ctx.optimizer.momentum_beta = trial.suggest_float("momentum_beta", 1e-4, 1, log=True)
        ctx.dims.sizes.full_conv_kernel = trial.suggest_int("full_conv_kernel", 1, 128, log=True)
        ctx.dims.sizes.depthwise_conv_kernel = trial.suggest_int("depthwise_conv_kernel", 1, 2048, log=True)
        ctx.dims.sizes.depth = trial.suggest_int("depth", 1, 256, log=True)
        ctx.dims.sizes.features_per_head = 128 * trial.suggest_int("features_per_head//128", 1, 8, log=True)
        ctx.dims.sizes.batch = 2 * trial.suggest_int("batch//2", 1, 128, log=True)
        ...
        ctx.dims.sizes.intermediate *= trial.suggest_int("group_linear_factor", 1, 32, log=True)
        ctx.model.leaky_relu_slope = trial.suggest_float("leaky_relu_slope", 1e-3, 2, log=True)
        ctx.model.weight_sharing = trial.suggest_int("weight_sharing", 0, 1)
