...
class WandbLog:
    ...
    def __call__(self, wctx: WhileTrainContext, step: int, current_lr) -> bool:
        ...
        items.update(self._log("Consistency Loss", wctx.scalars[0, 0], sizes))
        items.update(self._log("Loss", wctx.scalars[0, 1], sizes))
        items.update(self._log("Accuracy", wctx.scalars[0, 2], sizes))
        items.update(self._log("EMA Loss", wctx.scalars[0, 3], sizes))
        items.update(self._log("EMA Accuracy", wctx.scalars[0, 4], sizes))
